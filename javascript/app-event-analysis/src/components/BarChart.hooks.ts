...
function useHandleBarMouseLeave(
	hideTooltip: (force?: boolean) => boolean,
	hoverInfo: HoverInfo,
) {
	return useCallback(
		(event: React.MouseEvent) => {
			...
			d3.selectAll('.bar').attr('opacity', d => d.opacity || BAR_TRANSPARENT)
...
function useHandleContainerClick(hideTooltip: (force?: boolean) => boolean) {
	return useCallback(
		(event: React.MouseEvent) => {
