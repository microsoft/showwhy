...
from backend.exposure.worker.significance_test_task import (
...
async def significance_test(workspace_name: str, body: SignificanceTestRequestBody):
    ...
    propensity_score_async = chord(
        [calculate_propensity_score.s(estimate) for estimate in filtered_estimates]
    ...
    if len(results) < total:
        ...
        return StatusModel(
            ...
            results=SignificanceTestResult(
                p_value=p_value_str, significance=significance
            ),
        )
...
async def get_number_of_executions(
    workspace_name: str, body: SignificanceTestRequestBody
):
