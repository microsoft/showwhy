...
from server.rpy import exec_estimator_R
from server.types import Data, OutputLines, OutputResult, WeightedSynthdidControls

app = FastAPI()

if os.getenv("ENABLE_LOCAL_CORS", "").lower().strip() == "true":
    logging.info("Enabling local CORS for dev")
    origins = [
        "http://localhost:8080",
        "http://localhost:3000",
        "http://localhost:3002",
