...
def orchestrator_function(context: df.DurableOrchestrationContext):
    data = {**context.get_input(), "execution_id": context.parent_instance_id}
    ...
    for size, current, batch in get_batch(inference_inputs, ESTIMATE_EFFECT_BATCH_SIZE):
        tasks = [
            context.call_activity("InferenceExecuteActivity", task_inputs)
    ...
    inference_results = [
        {"output_file": result["output_file"], "task_id": result["task_id"]}
        for result in inference_results
        if result["output_file"] is not None
    ]

    # Refute Estimated Effect
    refuter_inputs = yield context.call_activity(
        "RefuteGeneratorActivity",
        {
    ...
    if data["node_data"]["confidence_interval"]:
        ...
        confidence_inputs = yield context.call_activity(
            "ConfidenceIntervalGeneratorActivity",
            {
                "session_id": data["session_id"],
                "inference_results": inference_results,
                "execution_id": context.parent_instance_id,
        ...
        for size, current, batch in get_batch(
            confidence_inputs, CONFIDENCE_INTERVAL_BATCH_SIZE
        ):
            tasks = [
                context.call_activity("ConfidenceIntervalActivity", task_inputs)
                for task_inputs in batch
            ]
            partial_results = yield context.task_all(tasks)
    ...
    for size, current, batch in get_batch(refuter_inputs, REFUTER_BATCH_SIZE):
        tasks = [
            context.call_activity("RefuteActivity", task_inputs)
            for task_inputs in batch
        ]
        partial_results = yield context.task_all(tasks)
        confidence_refuter_results.extend(partial_results)
    ...
    join_result = yield context.call_activity(
        "InferenceJoinResultActivity",
        {
            "session_id": data["session_id"],
    ...
    context.set_custom_status({"step": "Running SHAP Analysis"})

    final_result = yield context.call_activity(
        "ShapExecutionActivity",
        {
            "session_id": data["session_id"],
    ...
    cells = yield context.call_activity(
        "InferenceNotebookCellsActivity", data["node_data"]
    )
    ...
    context.set_custom_status({"step": "Completed"})

    return {**final_result, **notebook_result}
