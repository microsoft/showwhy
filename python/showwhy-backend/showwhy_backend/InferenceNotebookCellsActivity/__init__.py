...
def main(body: Dict) -> str:
    ...
    return [
        ...
        new_code_cell(
            [re.sub(r"from shared_code.*", "", inspect.getsource(inference_config))]
        ),
        new_code_cell(
        ...
        new_code_cell(
            [re.sub(r"from shared_code.*", "", inspect.getsource(confidence_interval))]
        ),
        new_code_cell(
            [
        ...
        new_code_cell(
            ["results_df = run_interpreter(results_df)", "results_df.head()"]
        ),
    ]
