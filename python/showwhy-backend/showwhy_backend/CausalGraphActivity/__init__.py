...
def main(body: Dict) -> str:
    ...
    causal_graph_nx = nx.cytoscape_graph(
        {"data": [], "directed": True, "multigraph": False, **node_data["causal_graph"]}
    )
    gml_graph = " ".join(nx.generate_gml(causal_graph_nx))

    if node_data["dataframe"] != "None":
        context[node_data["result"]] = CausalModel(
            data=context[node_data["dataframe"]],
    ...
    output = storage.write_output(
        session_id, node_data["result"], context[node_data["result"]], file_type="final"
    )

    storage.write_context(context)

    return {"output_file": output, "gml_graph": gml_graph}
