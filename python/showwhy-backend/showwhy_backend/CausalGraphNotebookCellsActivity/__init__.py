...
def main(body: Dict) -> str:
    ...
    cell_list = [
        ...
        new_code_cell(["import dowhy", "from IPython.display import Image, display"]),
    ]

    # if no real dataframe is passed on,
    # create dummy data with required columns to pass to doWhy
    model_data_name = "model_data"
    if node_data["dataframe"] == "None":
        columns = node_data["controls"] + [node_data["treatment"], node_data["outcome"]]
    ...
    cell_list.append(
        new_code_cell(
            [
                ...
                (
                    f"{node_data['result']} = dowhy.CausalModel(data={model_data_name},"
                    f"treatment='{node_data['treatment']}',"
                    f"outcome='{node_data['outcome']}',"
                    f"graph=gml_graph)"
                ),
