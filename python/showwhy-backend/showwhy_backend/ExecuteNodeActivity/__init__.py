...
def main(body: Dict):
    try:
        ...
        execution_instance: operations.OperationNode = getattr(
            operations, node_data['type'])(**node_data)
        ...
        if isinstance(result, pd.DataFrame) or result != None:
            is_final = 'final' if not 'partial' in body or not body[
                'partial'] else 'partial'
            name = storage.write_output(
                session_id, node_data['result'], result, file_type=is_final, extension='csv')

        try:
            json.dumps(result)
            return {
                'output_file': name if isinstance(result, pd.DataFrame) or result != None else None,
                'cells': cells,
                'result': result
            }
        except:
        ...
        logging.error(
            f"Something went wrong, input: {body}", exc_info=True)
        storage.write_output(session_id, f'error_{task_id}', {
            'error': str(ex),
