...
class CosmosDatabase(Database):
    ...
    def get_container(self):
        verify_connection = (
            False if os.environ.get("DISABLE_SSL_COSMOS", False) else True
        )

        client = CosmosClient.from_connection_string(
            os.environ["COSMOS_CONNECTION"],
            connection_verify=verify_connection,
            request_timeout=TIMEOUT,
        )

        db = client.create_database_if_not_exists(id=self.db_name)
...
class Mongo(Database):
    ...
    def get_records_for_execution_id(self, execution_id: str) -> List[Dict]:
        ...
        return [
            {key: value for key, value in item.items() if not key.startswith("_")}
            for item in container.find({"execution_id": execution_id})
        ]


def get_db_client():
    return (
        Mongo(DB_NAME, CONTAINER_NAME)
        if os.environ.get("MONGO_CONNECTION", False)
        else CosmosDatabase(DB_NAME, CONTAINER_NAME)
    )
