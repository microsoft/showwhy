...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    logging.info(f"Executing SignificanceTestOrchestrator with input: [{data}]")

    context.set_custom_status({"step": "Calculating effects under the null hypothesis"})

    # Estimate effects on null distribution
    significance_test_inputs = yield context.call_activity(
        "SignificanceTestGeneratorActivity", data
    )
    significance_test_results = []
    for size, current, batch in get_batch(
        significance_test_inputs, SIGNIFICANCE_BATCH_SIZE
    ):
        tasks = [
            context.call_activity("SignificanceTestExecuteActivity", task_inputs)
    ...
    final_result = yield context.call_activity(
        "SignificanceTestFinalResultActivity",
        {
    ...
    context.set_custom_status({"step": "Significance Test Completed"})
