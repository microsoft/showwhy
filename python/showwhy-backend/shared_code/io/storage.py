...
class StorageClient(ABC):
    ...
    @abstractmethod
    def write_output(self, session_id: str, name: str, output: Any, file_type: str = 'final', extension: str = None) -> None:
        pass
...
class BlobStorageClient(StorageClient):
    ...
    def read_context(self, session_id: str, name='metadata.pickle') -> Dict:
        try:
            ...
            blob_client = self.blob_service_client.get_blob_client(container="sessions",
                                                                   blob=os.path.join(session_id, name))
            data = blob_client.download_blob(
                timeout=self.timeout).content_as_bytes()
    ...
    def write_output(self, session_id: str, name: str, output: Any, file_type: str = 'final', extension: str = None):
    ...
    def get_download_url(self, session_id: str, name: str) -> str:
        ...
        _, _, account_key = storage_connection_string.split(
            ';')[account_key_index].partition('=')

        start_time = datetime.utcnow()
        end_time = start_time + timedelta(hours=1)

        token = generate_blob_sas(self.blob_service_client.account_name,
                                  'sessions',
                                  blob_path,
        ...
        return f'{self.blob_service_client.primary_endpoint}sessions/{blob_path}?{token}'
...
class LocalStorageClient(StorageClient):
    ...
    def write_context(self, context: Dict, name='metadata.pickle') -> None:
        try:
            path = os.path.join(self.local_storage_location,
                                context['session_id'])
    ...
    def read_context(self, session_id: str, name='metadata.pickle') -> Dict:
        try:
            with open(os.path.join(self.local_storage_location, session_id, name), 'rb') as context_file:
    ...
    def get_download_url(self, session_id: str, name: str) -> str:
        ...
        local_path = os.path.abspath(
            f'{self.local_storage_location}/{session_id}/final/{name}').replace('\\', '/')
        ...
        start_time = datetime.utcnow()
        end_time = start_time + timedelta(hours=1)
        token = generate_blob_sas(blob_service_client.account_name,
                                  'sessions',
                                  blob_path,
                                  account_key=account_key,
                                  start=start_time,
