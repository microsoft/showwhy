...
import pandas as pd
from shared_code.inference.inference import estimate_specification
    ...
    try:
        ...
        result_json = {
            **result,
            ...
            'task_id': f'{task_id}',
            'estimated_effect': result['estimated_effect'][2].value if result['estimated_effect'][2] else None
        }

        if result['estimated_effect'] == None:
            for refuter in node_data['refuter_specs']:
                result_json[f'refuter_{refuter["method_name"].replace("_refuter", "")}'] = None
