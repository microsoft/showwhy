...
Hypothesis: The median effect size in the observed curve is statistically different
    than in the null distribution.
Conditional permutation test:
    For each simulation:
    1. Compute propensity score based on confounders
    2. Stratify dataset based on propensity score (records within the same strata are
        likely to have similar confounders)
    3. Shuffle treatment values within each stratum
    4. Rerun specs using the bootstrapped sample, compute median effect
        of the null curve
    5. Repeat steps 1-4 for X simualations to generate a bootstrapped null
        curve distribution
    6. Compute p-value by comparing the observed median effect with the null
        curve metrics
"""
...
def __calculate_propensity_score(identified_estimand, causal_model, estimate):
    ...
    if (
        "propensity_score" not in causal_model._data.columns
        or "PropensityScoreWeighting" in (str(estimate.params["estimator_class"]))
    ):
        ...
        if len(observed_common_causes) > 0:
            ...
            propensity_model.fit(
                causal_model._data[observed_common_causes],
...
def calculate_all_propensity_scores(inference_results, num_strata=5):
    ...
    for result in inference_results:
        ...
        __calculate_propensity_score(identified_estimand, causal_model, estimate)
        if "propensity_score" in causal_model._data.columns:
            num_rows = causal_model._data.shape[0]
            causal_model._data["sig_strata"] = (
                (causal_model._data["propensity_score"].rank(ascending=True) / num_rows)
                * num_strata
            ).round(0)
...
def __generate_bootstrap_sample(identified_estimand, causal_model):
    ...
    if "sig_strata" in causal_model._data.columns:
        causal_model._data[treatment_var] = causal_model._data.groupby("sig_strata")[
            treatment_var
...
def perform_significance_test(inference_results, null_effects):
    """
    p-value: % of times that an equally or more extreme value was observed
        in the bootstrapped null curve distribution
    """
    original_effect = __calculate_original_effect(inference_results)
    extreme_results = [
        effect
        for effect in null_effects
    ...
    return {"p_value": p_value_str, "significance": 1 if p_value <= 0.05 else 0}
...
def __calculate_original_effect(inference_results):
    ...
    for result in inference_results:
        effect = (
            result["estimated_effect"][2].value
